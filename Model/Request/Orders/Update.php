...
class Update extends AbstractRequest
{
    ...
    protected function getParams()
    {
        ...
        foreach ($this->getOrdersCollection() as $order) {
            try {
                $this->_kimonixConfig->log("Request\Orders\Update::getParams() preparing order ID: {$order->getId()} ...", 'debug');
                $this->preparedParams[] = $this->kimonixSchema->getOrderSchema($order);
                $this->preparedOrdersCount++;
            } catch (\Exception $e) {
                $this->_kimonixConfig->log("Request\Orders\Update::getParams()- Exception: " . $e->getMessage() . "\n" . $e->getTraceAsString(), 'debug');
    ...
    {
        return RequestFactory::ORDERS_UPDATE_REQUEST_METHOD;
    }
    ...
    {
        return ResponseFactory::ORDERS_UPDATE_RESPONSE_HANDLER;
    }
}
