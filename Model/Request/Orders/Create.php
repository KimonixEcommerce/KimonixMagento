...
/**
 * Kimonix orders/create request model.
 */
class Create extends AbstractRequest
{
    ...
    protected function getParams()
    {
        ...
        foreach ($this->getOrdersCollection() as $order) {
            try {
                $this->_kimonixConfig->log("Request\Orders\Create::getParams() preparing order ID: {$order->getId()} ...", 'debug');
                $this->preparedParams[] = $this->kimonixSchema->getOrderSchema($order);
                $this->preparedOrdersCount++;
            } catch (\Exception $e) {
                $this->_kimonixConfig->log("Request\Orders\Create::getParams()- Exception: " . $e->getMessage() . "\n" . $e->getTraceAsString(), 'debug');
    ...
    {
        return RequestFactory::ORDERS_CREATE_REQUEST_METHOD;
    }
    ...
    {
        return ResponseFactory::ORDERS_CREATE_RESPONSE_HANDLER;
    }
}
