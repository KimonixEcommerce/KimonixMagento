...
/**
 * Kimonix products/create request model.
 */
class Create extends AbstractRequest
{
    ...
    protected function getParams()
    {
        ...
        foreach ($this->getProductsCollection() as $product) {
            try {
                $this->_kimonixConfig->log("Request\Products\Create::getParams() preparing product ID: {$product->getId()} ...", 'debug');
                $this->preparedParams[] = $this->kimonixSchema->getProductSchema($product);
                $this->preparedProductsCount++;
            } catch (\Exception $e) {
                $this->_kimonixConfig->log("Request\Products\Create::getParams()- Exception: " . $e->getMessage() . "\n" . $e->getTraceAsString(), 'debug');
    ...
    {
        return RequestFactory::PRODUCTS_CREATE_REQUEST_METHOD;
    }
    ...
    {
        return ResponseFactory::PRODUCTS_CREATE_RESPONSE_HANDLER;
    }
}
