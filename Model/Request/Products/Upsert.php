...
/**
 * Kimonix products/upsert request model.
 */
class Upsert extends AbstractRequest
{
    ...
    protected function getParams()
    {
        ...
        foreach ($this->getProductsCollection() as $product) {
            try {
                $this->_kimonixConfig->log("Request\Products\Upsert::getParams() preparing product ID: {$product->getId()} ...", 'debug');
                $this->preparedParams[] = $this->kimonixSchema->getProductSchema($product);
                $this->preparedProductsCount++;
            } catch (\Exception $e) {
                $this->_kimonixConfig->log("Request\Products\Upsert::getParams()- Exception: " . $e->getMessage() . "\n" . $e->getTraceAsString(), 'debug');
    ...
    {
        return RequestFactory::PRODUCTS_UPSERT_REQUEST_METHOD;
    }
    ...
    {
        return ResponseFactory::PRODUCTS_UPSERT_RESPONSE_HANDLER;
    }
}
